...
async def create_invoice(invoice: invoice_schema.InvoiceCreate):
    invoice_ref: DocumentReference = await invoice_crud.createInvoice(invoice=invoice)
...
async def read_invoice(invoice_id: str):
    invoice_ref = await invoice_crud.getInvoice(invoice_id=invoice_id)
    if invoice_ref is None:
...
async def update_invoice(invoice_id: str, invoice: invoice_schema.InvoiceBase):
    invoice_ref = await invoice_crud.updateInvoice(invoice_id, invoice.model_dump(exclude_unset=True))
    if invoice_ref is None:
        raise HTTPException(status_code=404, detail="Invoice not found")
    invoice_data = invoice_utils.json(invoice_ref)
    return invoice_schema.InvoiceDisplay(**invoice_data)
...
async def delete_invoice(invoice_id: str):
    invoice_ref = await invoice_crud.deleteInvoice(invoice_id)
    if invoice_ref is None:
        raise HTTPException(status_code=404, detail="Invoice not found")
